...
DockerClient::DockerClient(const std::string &socket_path) : socket_path_(socket_path)
{
...
std::string DockerClient::sendRequest(const std::string &method, const std::string &path, const std::string &body)
{
    CURL *curl;
    CURLcode res;
    std::string response;
    ...
    try
    {
        ...
        {
            throw std::runtime_error("HTTP error: " + std::to_string(http_code) + ", Response: " + response);
        }
    }
    catch (...)
    {
